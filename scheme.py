...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        result = scheme_eval(procedure.body,new_frame)
        return result
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Invalid number of variables and values')
        i = 0
        for formal in formals:
            frame.define(formal,vals[i])
            i += 1
        return frame
...
def do_mu_form(vals):
    ...
    return MuProcedure(formals, Pair('begin', vals.second),env)
...
def do_let_form(vals, env):
    ...
    for binding in bindings:
        if len(binding) > 2:
            raise SchemeError("Too many values passed in")
    ...
    for binding, value in bindings:
        if not scheme_symbolp(binding):
            raise SchemeError("Invalid variable name")
        else:
            names = Pair(binding,names)
        if scheme_symbolp(value) or isinstance(value, Pair):
...
def do_or_form(vals, env):
    ...
    if vals == nil:
        return False
    elif scheme_false(vals[0]):
        return do_or_form(vals.second,env)
    elif scheme_numberp(vals[0]):
        return quote(vals[0])
    elif vals.first.first == 'quote':
        return vals.first
    elif scheme_symbolp(vals.first.first):
        value = scheme_eval(vals.first,env)
        return do_or_form(vals.second, env) if value == False else value
    return quote(vals[0])
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if scheme_true(test):
            "*** YOUR CODE HERE ***"
            if clause.first == 'else':
                return do_begin_form(Pair(True, clause.second),env)
            elif len(clause) == 2:
                return do_if_form(clause,env)
            elif len(clause) > 2:
...
def do_begin_form(vals, env):
    ...
    check_form(vals, 1)
    "*** YOUR CODE HERE ***"
    vals = [scheme_eval(val,env) for val in vals]
    return Pair('quote',Pair(vals[-1],nil))
...
def check_formals(formals):
    ...
    """
    "*** YOUR CODE HERE ***"
    valid = [scheme_symbolp(formal) for formal in formals]
    if False in valid:
        raise SchemeError('Invalid Paramater name')
    if len(formals) != len(set(formals)):
        raise SchemeError('Duplicated Formal Parameters')
